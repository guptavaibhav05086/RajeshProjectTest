...
import {BulkdiscountProductPricingBuyerComponent } from '../bulkdiscount-product-pricing-buyer/bulkdiscount-product-pricing-buyer.component'
...
@Component({
  selector: 'app-price-to-buyer',
  templateUrl: './price-to-buyer.component.html',
  styleUrls: ['./price-to-buyer.component.css']
})
export class PriceToBuyerComponent implements OnInit {
...
public inrFormatter = Utils.formatAmountInINR;
  ...
  constructor(private _productPricing:MsupplyproductpricingService,
    private _commonServices:MSupplyCommonDataService,private productSrvice: ProductserviceService,
    private _msupplyFormService:MsupplyFormRegistrationService,private toast: ToastService,
    ...
    this._productPricing.getProducts().subscribe(data=> 
      {
        ...
        this._productPricing.getProductsPricingBuyer().subscribe(pricing=> {
          //debugger;
 ...
 */
openBulkDiscount(prodId:any,itemId:string,zoneId:string){
  debugger;
  let dateRange;
  const modelBulkDiscount = this.modalService.open(BulkdiscountProductPricingBuyerComponent,{size:'lg'});
  let slectedProd = this.mappedProductList.filter(item=> item._id == prodId)[0];
  modelBulkDiscount.componentInstance.product = this.mappedProductList.filter(item=> item._id == prodId);
  modelBulkDiscount.componentInstance.name=prodId;
  let listProd=this.mappedProductList;
  modelBulkDiscount.componentInstance.ProductList = listProd;
  modelBulkDiscount.componentInstance.zoneId = zoneId;
  // modelBulkDiscount.componentInstance.productPricingZone = this.priceMappingList;
  modelBulkDiscount.result.then(result => {
debugger;
let selprod =  slectedProd.manuFacturerPriceMapping.zonePrices.filter(item => item.pricingZone._id == zoneId )[0];
selprod.bulkPurchaseDiscount = result;
  });
 ...
 */
  openEditValidatity(prodId:any) {
    ...
    modalRef.result.then(result=> 
      {
        debugger;
        let selecedtProduct = this.mappedProductList.filter(item => item._id == result.product._id)[0];
        selecedtProduct.manuFacturerPriceMapping = this.priceMappingList.filter(data=> data._id == result._id)[0];
        selecedtProduct.manuFacturerPriceMapping.zonePrices = 
        selecedtProduct.manuFacturerPriceMapping.zonePrices.filter(zoneItem =>  zoneItem.pricingZone.isActive);
        selecedtProduct.validityStart=
       this.getDateFormat(this.priceMappingList.filter(data=> data._id == result._id)[0].validityStartTime);
       selecedtProduct.validityEnd=
       this.getDateFormat(this.priceMappingList.filter(data=> data._id == result._id)[0].validityEndTime);
       selecedtProduct.IsNewPricing=false;
       selecedtProduct.IsSaveDisable=false;
       selecedtProduct.prodGST=null;
       selecedtProduct.startTime = this.getTimeFormat(this.priceMappingList.filter(data=> data._id == result._id)[0].validityStartTime);
       selecedtProduct.endTime = this.getTimeFormat(this.priceMappingList.filter(data=> data._id == result._id)[0].validityEndTime)
       selecedtProduct.manuFacturerPriceMapping.zonePrices.forEach(zonePrice => {
        if(zonePrice.pricingSheetUrl !=null && zonePrice.pricingSheetUrl !=""){
          let arraySplit = zonePrice.pricingSheetUrl.split('/');
          zonePrice.pricingSheetName = arraySplit[arraySplit.length-1]; 
        }
  ...
  openPriceHistory(prodId:any,zoneId:any,manuPricingId:string,pricingZoneName:string) {
    ...
    if(manuPricingId != undefined){
      this._productPricing.getViewHistoryBuyer(manuPricingId,zoneId).subscribe(result => {
    ...
    err=> {console.log(err);this.toast.error('Error in getting History Data.Please contact admin')} );
    }
    else{
  ...
  copySelectedProductsRate(currentProdId:string,slectedProd:string){
    ...
    if(baseProduct != null){
      //for(let j=0;j < slectedProd.length; j++){
        ...
        //New object for copying base product prices to selected product
        ...
        for(let k=0 ; k < baseProduct.manuFacturerPriceMapping.zonePrices.length ; k++){
          ...
          //if(zoneDatatoCopy !=null){
            try {
              ...
              zoneDatatoCopy.bulkPurchaseDiscount=baseProductZone.bulkPurchaseDiscount;
              zoneDatatoCopy.isAddedNow = baseProductZone.isAddedNow;
        ...
        //Assinging the product pricing schema to the copied project
        productCopy.manuFacturerPriceMapping = objProdPricinng;
        console.log(productCopy.manuFacturerPriceMapping);
     ...
     // }
      ...
      this.toast.error('Error in copying rates.Please contact admin');
...
filterBrand(brandId:string){
  ...
  removeFile(productid:string,zoneId:string){
    let zonePrice=this.mappedProductList.filter(item=> item._id == productid)[0].
        ...
        zonePrice.pricingSheetUrl='';
        zonePrice.pricingSheetName = '';
  }
 ...
 * @returns       Comment for return value.
 */
UploadPricingSheet(files:File,productid:string,zoneId:string){
  let zonePrice=this.mappedProductList.filter(item=> item._id == productid)[0].
        manuFacturerPriceMapping.zonePrices.filter(price=> price.pricingZone._id == zoneId)[0];
  ...
  if(file.type.match('image.*') || ext=="doc" || ext=="docx" || ext =="pdf"|| ext=="xls" || ext=="xlsx"  ){
      ...
      this._msupplyFormService.postMsupplyFiles(file,fileName).subscribe(data=>{
        this.toast.success('File Uploaded Successfully');
          debugger;           
              zonePrice.pricingSheetUrl=data.url;
              zonePrice.pricingSheetName = fileName;
              //ZonePrice.pricingSheetName = 
  ...
  if(this.ValidateForm(productId)){
    let clickedProduct = this.mappedProductList.filter(item=> item._id == productId)[0];
      ...
      || clickedProduct.startTime == undefined || clickedProduct.endTime== undefined){
        this.toast.error('Provide Value for validity dates and Times');
        //alert('Provide Value for validity dates and Times');
        return;
    ...
    if(clickedProduct.IsNewPricing){
      ...
      if(this.validateDateRange(productId,"create")){
        debugger;
        this.loading =true;
        this._productPricing.createBuyerPricingZone(clickedProduct.manuFacturerPriceMapping).
      subscribe(result=> {
        this.loading =false;
        debugger;
        if(result.errors !=null){
          ...
          this._productPricing.getProductPriceBuyer(result.data._id).subscribe(value => 
            {
      ...
      this._productPricing.updateBuyerPricingZone(clickedProduct.manuFacturerPriceMapping).
      subscribe(result=> {
        console.log(result);
        if(result.errors !=null){
          this.toast.error('Error in Saving Details.Please contact admin');
...
try {
  ...
  let bindProduct = new Product();
    bindProduct._id = prodSelected._id;
    newProductPricing.product = bindProduct;
  //newProductPricing.product = prodSelected;
  newProductPricing.zonePrices = new Array<ZonePrice>();
